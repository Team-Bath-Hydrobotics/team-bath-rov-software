...
def main():
    ...
    parser.add_argument(
        "--source", type=str, required=True, help="Path to image or directory of images"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Detection mode (binary/multi)",
    )
    parser.add_argument(
        "--output", type=str, default="output_predictions", help="Output directory"
    )
    ...
    if os.path.isdir(args.source):
        images = glob.glob(
            os.path.join(args.source, "*.[jJ][pP]*[gG]")
        )  # matches .jpg, .jpeg, .png etc
