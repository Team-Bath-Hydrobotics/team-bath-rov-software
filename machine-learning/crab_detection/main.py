...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="output", help="Path to output directory"
    )
    parser.add_argument(
        "--display", action="store_true", help="Display image during processing"
    )
    parser.add_argument(
        "--mode",
        type=str,
        ...
        help="Detection mode: binary (Green/Not) or multi (Green/Jonah/Rock)",
    )
    ...
    pipeline = CrabPipeline(
        model_type=args.model, model_path=args.weights, mode=args.mode
    )
    ...
    if os.path.isdir(source):
        images = glob.glob(os.path.join(source, "*.[jJ]*")) + glob.glob(
            os.path.join(source, "*.[pP][nN][gG]")
        )
    else:
        images = [source]
